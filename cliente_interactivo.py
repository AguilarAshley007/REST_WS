...
def consultar_resultado():
    ...
    response = requests.get(
        f"{BASE_URL}/resultado/{request_id}",
        headers=get_headers(),
        verify="certs/server.crt"
    )
...
def descargar_certificado():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            ruta = "certs/server.crt"

            with open(ruta, "wb") as archivo:
                archivo.write(response.content)

            print(f"Certificado descargado correctamente en: {ruta}")

        else:
            print("Error al descargar el certificado.")
...
def menu():
    while True:
        ...
        print("9. Descargar Certificado SSL")
        print("10. Salir")
